...
const IndividualMeal: NextPageWithLayout<Props> = () => {
    ...
    const { type, day } = router.query as {
    ...
    const { data: meals } = api.meals.getMealsByComplexQuery.useQuery(
        { ...values, type: type.toLowerCase() },
        {
            ...
            onSuccess: (data) => {
                ...
                router.push(
                    "/add-new-meals/results?day=" + day + "&type=" + type
                );
            },
            onError: (error) => {
                setIsSubmitting(false);
    ...
    return (
        <div className="flex items-center justify-center w-full h-full gap-20 mt-10">
            ...
            <Formik
                ...
                <Form className="grid grid-cols-1 rounded border-box bg-base-200 md:grid-cols-2">
                    ...
                    <button
                        type="submit"
                        className="col-span-2 btn-primary btn">
                        Submit
                    </button>
                </Form>
            </Formik>
